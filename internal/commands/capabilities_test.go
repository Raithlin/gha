...
func TestCapabilitiesCommandProvidesCompleteVersionedInventory(t *testing.T) {
	...
	require.Len(t, capabilities.Commands, 22)
	assert.Contains(t, capabilities.Commands, model.Capability{Command: "agent install", Status: "available", ReadOnly: false, Notes: "Installs bundled gha skills and supported guidance for Codex, Claude Code, Pi, OpenCode, GitHub Copilot, or Gemini CLI; accepts comma-separated agents and records configured destinations; supports --dry-run and requires --confirm to write."})
	...
	assert.Contains(t, capabilities.Commands, model.Capability{Command: "agent uninstall", Status: "available", ReadOnly: false, Notes: "Removes GHA-managed guidance and skills for selected configured agents while retaining destinations shared by other configured agents; accepts comma-separated agents, supports --dry-run, and requires --confirm to write."})
