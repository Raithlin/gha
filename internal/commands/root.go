...
into a single cohesive experience.`,
	...
	root.AddCommand(newDashboardCmd(), newBranchesCmd(branchService), newPRsCmd(reviewService, resolver), newReleaseCmd(reviewService, resolver), newReviewCmd(reviewService, resolver))
	return root
}
