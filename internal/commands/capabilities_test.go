...
func TestCapabilitiesCommandProvidesCompleteVersionedInventory(t *testing.T) {
	...
	require.Len(t, capabilities.Commands, 17)
	...
	assert.Contains(t, capabilities.Commands, model.Capability{Command: "agent uninstall", Status: "available", ReadOnly: false, Notes: "Removes only managed GHA guidance; preserves installed skills and other instructions; supports --dry-run and requires --confirm to write."})
