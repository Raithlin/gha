...
into a single cohesive experience.`,
	...
	root.AddCommand(newCapabilitiesCmd(), newDashboardCmd(), newBranchesCmd(branchService), newBranchCmd(branchService, resolver), newPRsCmd(reviewService, resolver), newReleaseCmd(reviewService, resolver), newReviewCmd(reviewService, resolver))
	return root
}
